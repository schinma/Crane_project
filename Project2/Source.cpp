...
static float rotate;
static float X_translation;
static float Y_translation;
static int cube_view;
...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (ImGui::Begin("First Window")) {
			...
			ImGui::SliderFloat("Boom Rotation", &rotate, 0.0f, 360.0f);
			ImGui::SliderFloat("Box Horizontal", &X_translation, 0.0f, 1.0f);
			ImGui::SliderFloat("Box Vertical", &Y_translation, 0.0f, 1.0f);
	
			ImGui::RadioButton("Default", &cube_view, 0);
			ImGui::SameLine();
			ImGui::RadioButton("Box", &cube_view, 1);

			ImGui::End();
		}
		win->draw(rotate, X_translation, Y_translation, cube_view);
